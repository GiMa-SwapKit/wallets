...
import { type QuoteResponseRoute, SwapKitApi, type SwapRouteV2Response, TxnStatus } from "@swapkit/helpers/api";
import { ArrowDownUpIcon, HistoryIcon, Loader2Icon, LogOutIcon, Wallet2Icon } from "lucide-react";
...
import { applyColorsToElement, type SwapKitColors } from "./components/config";
...
export type SwapKitWidgetProps = {
  ...
  className?: string;
  /** Custom background colors for theming. All values in HSL format: "H S% L%" or "H S% L% / A" */
  colors?: SwapKitColors;
...
function SwapKitWidgetInner({
  ...
  // Apply colors via DOM manipulation to ensure proper CSS variable cascading
  // (derived variables like --sk-ui-card reference base variables like --sk-bg-surface)
  useEffect(() => {
    if (widgetRef.current && colors) {
      applyColorsToElement(widgetRef.current, colors);
    }
  }, [colors]);
  ...
  // from a sibling effect (notably useSwapTo below, declared earlier in this
  // component) finds populated headers. SKConfig is a zustand store — sync
  ...
  }, [inputAsset, outputAsset, fetchTokenPrices]);
  ...
  const performSwap = async (route: QuoteResponseRoute) => {
    ...
    try {
      ...
      if (routeWithTx.approvalTx) {
        if (!EVM_CHAINS.includes(sourceAsset.chain as (typeof EVM_CHAINS)[number])) {
            ...
            {!isWalletConnected ? (
              ...
              <div className="sk-ui-ml-2 sk-ui-flex sk-ui-items-center sk-ui-gap-px">
                <Button
                  ...
                  onClick={() => {
                    void showSwapKitWalletDrawer({ inputAssetChain });
                  }}
                  size="xs"
                  variant="secondary">
