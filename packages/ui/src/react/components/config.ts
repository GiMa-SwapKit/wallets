...
/**
 * Theme colors for the SwapKit widget.
 * All values should be in HSL format: "H S% L%" or "H S% L% / A"
 *
 * Note: These map to dedicated theme variables (--sk-bg-*, --sk-ui-accent, etc.)
 * to avoid conflicts with component-level styles.
 */
export interface SwapKitColors {
...
export const DEFAULT_COLORS: Required<SwapKitColors> = {
  accent: "140 87% 79%",
...
/** CSS variable names mapped to SwapKitColors keys */
const COLOR_VAR_MAP: Record<keyof SwapKitColors, string> = {
  accent: "--sk-ui-accent",
...
/**
 * Applies custom colors to CSS variables on a DOM element.
 * Only sets variables for colors that are provided.
 ...
 */
export function applyColorsToElement(element: HTMLElement, colors: SwapKitColors) {
  for (const [key, value] of Object.entries(colors)) {
    if (value) {
      const varName = COLOR_VAR_MAP[key as keyof SwapKitColors];
 ...
 * Returns CSS custom property declarations for provided colors.
 */
export function getColorsStyleProps(colors?: SwapKitColors): React.CSSProperties {
  if (!colors) return {};
