...
describe("empty swapTo edge cases", () => {
  test("empty swapToAssets with filterBySwapTo=true returns full static list", () => {
    const assetsByChain = makeAssetsByChain(STATIC_IDENTIFIERS);
  ...
  test("empty swapToAssets with network filter still returns filtered static assets", () => {
    const assetsByChain = makeAssetsByChain(STATIC_IDENTIFIERS);
