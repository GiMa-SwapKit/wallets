...
import { ChevronDown, LogOut, RefreshCw, Search, Wallet2 } from "lucide-react";
...
function ChainSection({ chain, balances }: { chain: Chain; balances: BalanceDetails[] }) {
      ...
      {open && (
        <div className="sk-ui-space-y-4 sk-ui-pb-2">
          {chains.map(([chain, balances]) => (
            <ChainSection balances={balances} chain={chain} key={`wallet-chain-${chain}`} />
          ))}
        </div>
      )}
    </div>
