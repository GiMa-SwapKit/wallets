...
export async function mayachainWalletMethods({
  ...
  const { createStargateClient, getCosmosToolbox } = await import("@swapkit-dev/toolboxes/cosmos");
  ...
  const signTransaction = async ({ assetValue, recipient, sender, memo }: SignTransactionParams) => {
    ...
    const { getDenomWithChain } = await import("@swapkit-dev/toolboxes/cosmos");
